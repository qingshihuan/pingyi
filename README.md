...
打开 **设置 → 外观与启动 → 全局快捷键**，直接输入组合，或点击“录制快捷键”后按下并松开组合；点击“应用快捷键”立即注册并保存。“恢复默认”先修改待应用值，还需点击应用。支持 Ctrl/Alt/Shift 加一个字母或数字；Esc、Tab、失焦或关窗可取消录制。注册或写入失败恢复旧键，回滚失败明确提示，不影响按钮截图。

Linux 默认 `Ctrl+Shift+D` 未列入 Ubuntu/GNOME 官方常用系统默认快捷键表，但用户自定义绑定和应用内命令仍可能重叠，例如 Firefox 的“收藏所有标签页”。不能承诺所有 Linux 环境都无冲突。Wayland 此处只保存偏好组合，实际绑定需在系统键盘设置中完成。操作、来源、升级迁移及验证边界见 [快捷键说明](docs/HOTKEYS.md)。
...
安装后启动屏译，按默认快捷键（Windows：`Ctrl+Alt+D`；Linux X11：`Ctrl+Shift+D`）或自己保存的组合，拖动鼠标框选屏幕区域。多显示器会按各自 DPI 建立协同截图层，支持负坐标和跨屏框选。识别完成后可以复制原文、复制译文、复制全部、重试或固定结果卡；后续截图复用同一结果窗口，不会持续堆叠窗口。快捷键、OCR/翻译提供商、本地模型和凭据均在“设置”中管理；密钥只提供显示/隐藏控制，显示后可在输入框内使用标准复制和粘贴快捷键。
...
工作台支持 `Ctrl+,` 打开设置、`F5` 刷新状态；设置窗口支持 `Ctrl+S` 保存（录制快捷键期间除外）。Windows 默认截图键为 `Ctrl+Alt+D`，Linux 为 `Ctrl+Shift+D`；旧 Linux 默认按版本迁移，其他自定义组合保留。当前源码统一为单一主界面，设置单独打开，不再提供重复的“经典界面”切换。
...
需要直接打开设置窗口排障时，可在可执行文件后添加 `--settings`；使用 `--capture` 可在首次启动时截图，或把截图命令转发给已经运行的实例。
...
Linux X11 默认使用 `Ctrl+Shift+D`。Schema 10 迁移未标记自定义的旧 Linux 默认；schema 9 中已主动改成 `Ctrl+Alt+D` 的设置和其他自定义组合保留。旧版本没有显式自定义标记，同旧默认完全一致的自定义意图无法区分；详见 [快捷键迁移说明](docs/HOTKEYS.md)。注册冲突会明确提示，不再导致 Xlib 终止程序；截图按钮不依赖全局快捷键。
...
Wayland 全局绑定由系统管理：在“设置 → 外观与启动”保存偏好并复制截图命令，到 Ubuntu“设置 → 键盘 → 自定义快捷键”中绑定，例如 `Ctrl+Shift+D`。应用中保存偏好不会自动注册、改动或覆盖系统快捷键；首次启动和已运行时的 `--capture` 都进入截图流程。桌面启动器也提供“截图翻译”和“设置”操作。
